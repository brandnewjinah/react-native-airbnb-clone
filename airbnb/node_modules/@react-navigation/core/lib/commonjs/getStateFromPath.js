...
function getStateFromPath(path, options = {}) {
  ...
  const configs = [].concat(...Object.keys(options).map(key => createNormalizedConfigs(key, options, [], initialRoutes))).sort((a, b) => {
    // Sort config so that:
  ...
  let result;
  let current; // We try to match the paths in 2 passes
